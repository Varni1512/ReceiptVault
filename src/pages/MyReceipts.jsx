      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-end mb-6 border-b border-slate-200 pb-6">
          ...
          <button 
            ...
            className="bg-slate-900 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-blue-600 transition-all shadow-lg active:scale-95"
          >
            <Plus size={20} />
        ...
        <div className="py-10"> 
                ...
                <div className="h-56 bg-slate-100 relative overflow-hidden">
                  ...
                  <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-4 group-hover:translate-x-0">
                    <button 
                      ...
                      className="bg-white p-2 rounded-lg text-slate-900 hover:bg-blue-600 hover:text-white shadow-md"
                    >
                      <Maximize2 size={18} />
                    </button>
                    <button 
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-3 mt-12">
              <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="p-2 rounded-xl border bg-white disabled:opacity-30"><ChevronLeft size={20} /></button>
              <div className="flex gap-2">
                {[...Array(totalPages)].map((_, i) => (
                  <button key={i} onClick={() => setCurrentPage(i + 1)} className={`w-10 h-10 rounded-xl font-bold text-sm ${currentPage === i + 1 ? 'bg-slate-900 text-white' : 'bg-white border text-slate-600'}`}>{i + 1}</button>
                ))}
              </div>
              <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="p-2 rounded-xl border bg-white disabled:opacity-30"><ChevronRight size={20} /></button>
            </div>
          )}
        </div>
            ...
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-6">
              ...
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative bg-white w-full max-w-md rounded-2xl shadow-2xl p-8">
                ...
                <form onSubmit={handleUploadSubmit} className="space-y-6">
                  ...
                  <button type="submit" className="w-full bg-slate-900 text-white font-bold py-4 rounded-xl hover:bg-blue-600 transition-all">Add to Vault</button>
                </form>
              </motion.div>
            </div>
          )}
              ...
              >
                ...
                <div className="p-4 bg-white border-b flex justify-between items-center px-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button 
                      onClick={() => setPreviewDoc(null)}
                      className="p-2 text-slate-400 hover:text-slate-900 transition-all bg-slate-100 rounded-lg"
                    >
                      <X size={20} />
                    </button>
                  </div>
